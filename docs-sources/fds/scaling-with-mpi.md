...
In this example, we're using a cloud machine (`c2d-standard-8`) equipped with 8 virtual CPUs. Explore the full range of available machines [here](https://console.inductiva.ai/machine-groups/instance-types).
...
In the previous example, we used hyperthreading by assigning one MPI process per virtual CPU.

By default, Google Cloud VMs provide 2 vCPUs per physical core, meaning hyperthreading is enabled. To disable it and use only physical CPU cores, configure the machine group with `threads_per_core=1`:
...
Compared to the previous 8-mesh run on a `c2d-standard-8` machine, this setup achieved a higher speed-up—closer to the theoretical maximum of 8×. However, since it uses a more expensive machine type, the overall cost increased slightly.
