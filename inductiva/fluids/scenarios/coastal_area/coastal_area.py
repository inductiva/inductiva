...
class Bathymetry:
    ...
    def from_random_depths(
        ...
        A grid of random depths of size (2^n + 1) x (2^n + 1) is created, where
        n is an integer.

        The depths are randomly generated as follows:
        1. A maximum depth value `max_depth` is used to specify the range of
        depths to assign to the corners of the grid. The corners on the lower x
        ...
        2. The depths of the remaining points are generated by applying the
        Diamond-Square algorithm. The randomness of the algorithm is controlled
        ...
        Args:
            n: Sets the size of the grid of random depths, (2^n + 1) x (2^n + 
              1).
            x_range: The range of x values, in meters.
            y_range: The range of y values, in meters.
