...
def register(machine_group):
    ...
    logging.info("■ Registering %s configurations (client-side):", machine_group.short_name())
    ...
    if isinstance(machine_group.max_idle_time, int):
        machine_group.max_idle_time = datetime.timedelta(minutes=machine_group.max_idle_time)
    
    machine_group._cpu_info = type('CPUInfo', (), {
        'cpu_cores_logical': _estimate_vcpus_from_machine_type(machine_group.machine_type),
        'cpu_cores_physical': _estimate_vcpus_from_machine_type(machine_group.machine_type) // 2
    })()
    ...
    machine_group._cost_per_hour = type('CostInfo', (), {
        'min': 0.1,
        'max': 0.1,
...
def _register_machine_group_backend(machine_group):
    ...
    instance_group_config = inductiva.client.models.RegisterVMGroupRequest(
        ...
        max_idle_time=machine_group._timedelta_to_seconds(machine_group.max_idle_time),
...
def _estimate_vcpus_from_machine_type(machine_type):
    ...
    raise ValueError(f"Could not extract vCPU count from machine type: {machine_type}")
...
def start(machine_group, verbose: bool = True):
    ...
    try:
        cmd = [
            ...
            '--zone', machine_group.zone,
            '--machine-type', machine_group.machine_type,
            '--image-family', 'ubuntu-2204-lts',
            '--image-project', 'ubuntu-os-cloud',
            '--scopes', 'https://www.googleapis.com/auth/cloud-platform',
            '--metadata', f'INDUCTIVA_API_KEY={api_key},INDUCTIVA_API_URL={api_url},MACHINE_GROUP_NAME={machine_group._name}',                                                                                              
        ...
        if verbose:
            print(f"Creating GCP VM '{machine_group._name}' in zone '{machine_group.zone}'...")
        ...
        if result.returncode == 0:
            ...
            logging.info("%s successfully started in %s.", machine_group, creation_time)
            
            if verbose:
                print("GCP VM created successfully!")
                print(f"VM Name: {machine_group._name}")
                print(f"Zone: {machine_group.zone}")
                print("The task-runner will start automatically once the VM is ready.")
...
def terminate(machine_group, verbose: bool = True):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            try:
                machine_group._api.delete_vm_group(machine_group_id=machine_group.id)
            except Exception as e:
            ...
            machine_group._client_vm_info['terminated_at'] = datetime.datetime.now()
            
            logging.info("%s terminated.", machine_group)
            
            if verbose:
                print(f"GCP VM '{machine_group._name}' terminated successfully.")
            ...
            if "not found" in result.stderr.lower():
                ...
                if verbose:
                    print(f"GCP VM '{machine_group._name}' was already terminated.")
                return True
            else:
