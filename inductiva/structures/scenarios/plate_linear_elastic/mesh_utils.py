    ...
    def _create_mesh_with_gmsh(self) -> None:
        ...
        1. Define the distance-based mesh size field:
            Use gmsh.model.mesh.field.add("Distance", mesh_field_id) to add a
              distance-based mesh size field, where mesh_field_id is a unique
              identifier for the field.
            Set the curves or points where you want to apply the distance-based
              field using gmsh.model.mesh.field.setNumbers(field_id, 
              "CurvesList", curves_list) or 
              gmsh.model.mesh.field.setNumbers(field_id, "PointsList", 
              points_list). 
            Adjust the points_list of the field 
              (i.e., how many points to use along curves or points) with
        ...
        2. Define the threshold-based mesh size field:
            Use gmsh.model.mesh.field.add("Threshold", mesh_field_id) to add a
              threshold-based mesh size field, where mesh_field_id is a unique
              identifier for the field.
            Use gmsh.model.mesh.field.setNumber(mesh_field_id, "InField",
              ref_mesh_field_id): The "InField" option specifies that this new 
              field will use the values from another existing field to compute 
              its own values. In this case, it uses the same field with an 
              identifier ref_mesh_field_id as a basis for computation. This 
              means that the new field will be based on the values of another 
              field with an identifier one less than the current mesh_field_id.
              The "InField" option is useful for creating hierarchical mesh size
              fields, where one field depends on another, allowing more control
              over mesh refinement strategies.
        ...
        3. Combine the distance and threshold-based fields:
            ...
            Use gmsh.model.mesh.field.add("Min", combined_mesh_field_id) to add
              a combined field that uses the minimum element size from the 
        ...
        5. Generate the mesh:
            Finally, use gmsh.model.mesh.generate(dim=2) to generate the 2D mesh
              based on the defined mesh size fields and other meshing options.

        We can define both the global mesh size and the local mesh size 
          (around the holes).
        """

        def add_mesh_field_distance(mesh_field_id: int, curves_list: List[int],
                                    num_points: int) -> None:
            """Add a distance-based mesh size field to control element size in 
              Gmsh.
            ...
            gmsh.model.mesh.field.setNumbers(mesh_field_id, "CurvesList",
                                             curves_list)

            # Adjust the points_list of the field to control mesh size along
            # curves
            gmsh.model.mesh.field.setNumber(mesh_field_id, "Sampling",
                                            num_points)
        ...
        def add_mesh_field_threshold(mesh_field_id: int, ref_mesh_field_id: int,
            ...
            """Add a threshold-based mesh size field to control element size in
              Gmsh.
            ...
            #  values
            gmsh.model.mesh.field.setNumber(mesh_field_id, "InField",
                                            ref_mesh_field_id)
