...
@pytest.mark.parametrize(
    "command_template, render_args, command_output",
    [("grep {{ name }} {{ filename }}", dict(
        name="time", filename="out.log"), "grep time out.log"),
     ("echo inductiva is awesome > awesome.txt", dict(),
...
@pytest.mark.parametrize(
    ...
    [(os.path.join(os.path.dirname(__file__), "..", "assets",
                   "commands_template.json.jinja"), dict(name="time"),
      os.path.join(os.path.dirname(__file__), "..", "assets",
...
def test_render_file(file, render_args, output_file, expected_output):
    ...
    commands_file = open(output_file, "r", encoding="utf-8")
...
@pytest.mark.parametrize(
    ...
    [(os.path.join(os.path.dirname(__file__), "..", "assets", "template_dir"),
      dict(simulator="openfoam", simulator_version="esi"),
...
def test_render_directory(template_dir, render_args, target_dir,
    ...
    template_dict = json.load(open(target_file, "r", encoding="utf-8"))
